...
static void drawmenu(void) {
  ...
  if (lines > 0) {
    ...
    for (item = curr, y = bh + height_separator; item != next;
         item = item->right, y += bh) {
      drw_setscheme(drw,
                    (item == sel) ? scheme[SchemeSel] : scheme[SchemeNorm]);
...
static void setup(void) {
  ...
  bh = centered ? MAX(drw->fonts->h + 2, lineheight) : drw->fonts->h + 2;
